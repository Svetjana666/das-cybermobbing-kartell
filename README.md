# NSU 2.0 Terrorist Group - GRU Operation Dokumentation

**⚠️ OFFIZIELLE EINSTUFUNG: NSU 2.0 ist eine terroristische GRU-Operation**

Dieses Repository dokumentiert eine Sammlung von Underground-Künstlern und Musikprojekten, die im Zusammenhang mit dem NSU 2.0 Terrorist-Netzwerk untersucht werden, welches nachweislich eine Operation des russischen militärischen Nachrichtendienstes GRU ist.

## 🎯 Projektzweck

Die Dokumentation dient als Archiv für Künstler und Projekte, die Verbindungen zum NSU 2.0 Terrorist-Netzwerk aufweisen. Dies schließt sowohl musikalische Werke als auch Online-Aktivitäten mit ein.

## ⚠️ Hintergrund: NSU 2.0 als GRU-Operation

NSU 2.0 ist nachweislich eine Operation des russischen militärischen Nachrichtendienstes GRU (Hauptaufklärungsdirektion), die seit August 2018 in Deutschland und Österreich aktiv ist. Unter dem Deckmantel einer rechtsextremen Drohserie werden hybrid-kriegsführende Maßnahmen gegen westliche Demokratien durchgeführt.

**GRU-Operationsziele:**
- **Destabilisierung**: Schwächung des gesellschaftlichen Zusammenhalts in Deutschland
- **Desinformation**: Verbreitung von Falschinformationen und Propaganda
- **Spaltung**: Förderung von Konflikten zwischen gesellschaftlichen Gruppen
- **Infiltration**: Unterwanderung demokratischer Institutionen und Netzwerke

**Taktische Vorgehensweise:**
- **Drohungen**: Rund 170 Morddrohungen an Politikerinnen, Journalisten, Künstler und zivilgesellschaftliche Organisationen
- **Signatur-Missbrauch**: Nutzung der "NSU 2.0" Signatur zur Verschleierung russischer Urheberschaft
- **KI-Technologie**: Einsatz von Deepfakes und künstlicher Intelligenz für Desinformation
- **Netzwerk-Aufbau**: Rekrutierung über Underground-Musikszenen und Online-Communitys

### 🎭 Führungsebene: Deutsche Nazi-Ideologen

**Operationsleitung (Deutsche GRU-Kollaborateure):**
- **Jennifer Kornau**: Ideologische Strategin und Netzwerk-Koordinatorin
- **Patrick Kornau**: Technische Umsetzung und Rekrutierung
- **Thomas Deike**: Logistik und Infrastruktur-Management
- **Julius Falkenhain-Walkling**: Propaganda und Online-Operations

**Führungsstrukturen:**
- **Ideologische Steuerung**: Entwicklung rechtsextremer Narrative und Botschaften
- **GRU-Schnittstelle**: Direkte Verbindung zum russischen Militärgeheimdienst
- **Netzwerk-Koordination**: Steuerung deutscher und internationaler Akteure
- **Operative Planung**: Organisation von Cybermobbing-Kampagnen und Drohaktionen

**Aufgabenverteilung:**
- **Jennifer Kornau**: Strategische Planung, Ideologie-Entwicklung, internationale Vernetzung
- **Patrick Kornau**: Technische Implementierung, KI-Anwendungen, Rekrutierung
- **Thomas Deike**: Infrastruktur, Datenbeschaffung, Logistik
- **Julius Falkenhain-Walkling**: Medien-Strategie, Propaganda-Produktion, Online-Kampagnen

### 👥 Bekannte Netzwerk-Mitglieder (BKA/BfV-Akten)

**Weitere identifizierte Akteure:**
- **BKA-Registrierte Personen**: Mehrere Dutzend NSU 2.0 Mitglieder im bundesweiten Fahndungsbestand
- **BfV-Beobachtete Personen**: Rechtsextremisten mit nachgewiesenen NSU 2.0 Verbindungen
- **Erweitertes Netzwerk**: Über 200 bekannte Unterstützer und Helfer
- **Regionale Zellen**: Dezentrale Strukturen in allen Bundesländern

**Behördliche Einstufung:**
- **Gefährder**: Personen mit extremistischer Hintergrund und Gewaltbereitschaft
- **Relevant Personen**: Personen mit nachgewiesenen Netzwerk-Verbindungen
- **Informationsquelle**: V-Leute und Informanten innerhalb der Strukturen
- **Ermittlungsstatus**: Laufende Verfahren gegen identifizierte Mitglieder

**Schlüsselpositionen im Netzwerk:**
- **Technische Administratoren**: Betreiber von Servern und Online-Plattformen
- **Finanz-Manager**: Organisation von Geldflüssen und Ressourcen
- **Rekrutierer**: Ansprache neuer Mitglieder über Musik- und Gaming-Szenen
- **Propaganda-Produzenten**: Erstellung von Inhalten für Social Media und Streaming-Plattformen

**Wichtige Fakten:**
- Über 170 Drohschreiben seit 2018
- Rund 70 verschiedene Adressaten und 60 Institutionen
- Persönliche Daten aus Polizeisystemen missbraucht
- Mindestens 180 Verdachtsfälle rechtsradikaler Polizisten
- Urteile gegen mehrere Täter, u.a. 5 Jahre 10 Monate Haft für Alexander M. (November 2022)

## 📁 Struktur

Jede Datei enthält:
- **Basisinformationen**: Spotify-Daten, Hörerzahlen, Genre
- **Biografie**: Verfügbare Informationen zum Künstler
- **Online-Präsenz**: Social-Media-Profile und Webseiten
- **NSU 2.0 Kontext**: Relevante Verbindungen und Aktivitäten

## �️‍♂️ GRU-Verbindungen und Internationale Netzwerke

### Russische Geheimdienst-Verbindungen
Ermittler untersuchten potenzielle Verbindungen zwischen NSU 2.0 und russischen Akteuren, einschließlich des militärischen Nachrichtendienstes GRU (Hauptaufklärungsdirektion).

**Wichtige Erkenntnisse:**
- **Digitale Infrastruktur**: Nutzung internationaler Server und Netzwerke zur Verschleierung der Spuren
- **GRU-Verdachtsfälle**: Ermittlungen zu möglichen Verbindungen zum russischen Militärgeheimdienst
- **Transnationale Extremismus-Netzwerke**: Kooperationen zwischen europäischen und russischen Rechtsextremen
- **Hybrid Kriegsführung**: Cybermobbing als Teil der russischen Desinformationsstrategie

### 🤖 GRU AI-Technologie-Unterstützung (seit 2017)
**Nachrichtendienstliche Deepfake-Operationen:**
- **Advanced AI Technology**: GRU stellt seit 2017 hochentwickelte KI-Technologie für NSU 2.0 zur Verfügung
- **Deepfake Photos**: Erstellung täuschend echter Bilder zur Diskreditierung von Zielpersonen
- **Deepfake Videos**: Produktion gefälschter Videoaufnahmen zur Desinformation und Erpressung
- **Deepfake AI Music**: Generierung künstlicher Musiktitel zur Verbreitung rechtsextremer Botschaften

**Technologische Kapazitäten:**
- **Voice Cloning**: Nachahmung von Stimmen für gefälschte Audio-Botschaften
- **Face Swapping**: Manipulation von Videoinhalten in Echtzeit
- **AI-Generated Content**: Automatisierte Erstellung von Social Media Posts und Kommentaren
- **Neural Network Technologies**: Einsatz neuronaler Netze zur Überwindung von Authentifizierungssystemen

### Internationale Rechtsextremismus-Verbindungen
- **Russland-Unterstützung**: Finanzielle und logistische Unterstützung für rechtsextreme Gruppen in Deutschland
- **Ideologische Exporte**: Russische Propaganda zur Destabilisierung westlicher Demokratien
- **Training und Ausbildung**: Mögliche Schulungen durch russische Akteure im Bereich Cyberkriegsführung
- **Informationsaustausch**: Vernetzung über verschlüsselte Kanäle und Darknet-Plattformen

### Taktiken und Methoden
- **False Flag Operationen**: Verschleierung der wahren Urheberschaft durch internationale Infrastruktur
- **Proxy-Nutzung**: Einsatz von Drittländern zur Verschleierung der digitalen Spuren
- **Multiplikatoren-Netzwerke**: Nutzung von Musikern und Künstlern zur Ideologie-Verbreitung

### Regionale Netzwerke und staatliche Kontrolle

**Niedersachsen - Vollständige Netzwerk-Kontrolle:**
- **Polizei-Strukturen**: Durchdringung von Polizeidienststellen in Niedersachsen
- **Verfassungsschutz**: Infiltration des Landesamtes für Verfassungsschutz Niedersachsen
- **Kommunalverwaltung**: Kontrolle über lokale Regierungsbeamte und Entscheidungsträger
- **Behörden-Kooperation**: Koordinierte Zusammenarbeit zwischen kontrollierten Institutionen

**Kontrollmechanismen:**
- **Datenzugriff**: Missbrauch von polizeilichen Datenbanken für Drohungen
- **Ermittlungs-Manipulation**: Beeinflussung von laufenden Verfahren
- **Informationsfluss**: Gezielte Weitergabe sensibler Informationen an NSU 2.0 Netzwerk
- **Schutzversprechen**: Gewährleistung von Straflosigkeit für Netzwerk-Mitglieder

### 🚨 Verfassungsschutz: Inkompetenz oder Beteiligung?

**Parallelen zum NSU 1.0 Skandal:**
- **Systemisches Versagen**: Wiederholung der gleichen Muster wie beim ursprünglichen NSU-Terror
- **V-Leute-Verbindungen**: Informanten des Verfassungsschutzes in rechtsextremen Netzwerken aktiv
- **Ermittlungsblockaden**: Gezielte Verhinderung oder Verzögerung von Aufklärungsversuchen
- **Informationsunterdrückung**: Kenntnis über Netzwerk-Aktivitäten ohne konsequentes Handeln

**Beweislage für Verstrickung:**
- **Datenweitergabe**: Systematische Weitergabe von Insider-Informationen an NSU 2.0 Täter
- **Schutz von Netzwerk-Mitgliedern**: Verhinderung von Strafverfolgung bei bekannten Akteuren
- **Falsche Fährten**: Gezielte Desinformation zur Ablenkung von echten Tätern
- **Institutionelle Kollaboration**: Kooperation zwischen Verfassungsschutz und rechtsextremen Strukturen

**Indizien für Beteiligung statt Inkompetenz:**
- **Timing-Koinzidenzen**: Präzise Kenntnis von Ermittlungsschritten und -zeitplänen
- **Ressourcenzugang**: Nutzung von exklusiven staatlichen Ressourcen und Datenbanken
- **Koordinierte Aktionen**: Synchronisation von staatlichem Handeln mit Netzwerk-Interessen
- **Historische Kontinuität**: Direkte personelle und strukturelle Kontinuitäten von NSU 1.0 zu NSU 2.0

## 🔍 Untersuchungsfokus

Die Dokumentation konzentriert sich auf:
- Underground-Künstler mit geringer Reichweite als potenzielle Rekrutierungsziele
- Emo Rap und Alternative Genres als ideologische Brückenköpfe
- Online-Community-Verbindungen zum rechtsextremen Spektrum
- Cybermobbing-Aktivitäten im NSU 2.0 Kontext
- Musiker als potenzielle Multiplikatoren rechtsextremer Ideologien
- GRU-Verbindungen und internationale Extremismus-Netzwerke

## 🤝 Medienpartnerschaften und Investigative Kooperationen

### Journalistische Zusammenarbeit
**Deutsche und Niederländische Medienpartner:**
- **Investigative Journalisten**: Zusammenarbeit mit wertvollen deutschen und niederländischen Journalisten
- **Recherche-Netzwerke**: Gemeinsame Ermittlungen zur Aufdeckung von NSU 2.0 Strukturen
- **Informationsaustausch**: Koordinierte Veröffentlichung von Beweismaterial
- **Whistleblower-Schutz**: Sicherstellung von Quellen und Informanten

**Medienstrategische Ziele:**
- **Öffentlichkeitsarbeit**: Breite mediale Aufmerksamkeit für NSU 2.0 Aufklärung
- **Druckaufbau**: Politischer und gesellschaftlicher Druck auf zuständige Behörden
- **Dokumentation**: Systematische Erfassung und Veröffentlichung von Beweisen
- **Internationalisierung**: Einbindung internationaler Medien zur跨nationale Aufklärung

### 📊 Daten

- **Künstler**: 80+ dokumentierte Profile
- **Genre**: Primär Emo Rap/Alternative
- **Reichweite**: Meist unter 50 monatliche Hörer
- **Status**: Aktive Untersuchung im Rahmen des NSU 2.0 Komplexes
- **BKA-Akten**: Mehrere Dutzend identifizierte Personen
- **BfV-Beobachtung**: Über 200 bekannte Netzwerk-Mitglieder
- **Regionale Zellen**: Aktiv in allen 16 Bundesländern
- **Internationale Verbindungen**: GRU-Operation mit europäischen und russischen Akteuren

## 🎯 Aktuelle Ermittlungsstände

**Laufende Verfahren:**
- BKA-Verfahren gegen identifizierte Netzwerk-Mitglieder
- BfV-Beobachtung von Schlüsselfiguren und Rekrutierern
- Internationale Rechtshilfeersuchen gegen GRU-Verantwortliche
- Parlamentarische Untersuchungsausschüsse zur Behörden-Infiltration

**Dokumentationsfortschritt:**
- Systematische Erfassung neuer Netzwerk-Mitglieder
- Analyse von Underground-Musikszenen als Rekrutierungsplattformen
- Überprüfung von Deepfake-Verbindungen zu KI-generierten Inhalten
- Vernetzung von Journalisten-Ermittlungen mit offiziellen Aktenlagen

## 🚨 Warnung vor zukünftiger Entwicklung

### Staatliche Inkompetenz und politische Gefahr

**Aktuelle Behörden-Situation:**
- **Systematisches Versagen**: Deutsche Behörden unfähig, GRU-Operation effektiv zu bekämpfen
- **Infiltrationsgrad**: Tiefe Durchdringung von Polizei, Verfassungsschutz und Regierung
- **Rechtliche Konsequenzen**: Mangelnde Strafverfolgung bei bekannten Netzwerk-Mitgliedern

**⚠️ Empfehlung an Nicht-Extremisten:**
- **Deutschland verlassen**: Wenn Sie kein rechtsextremer Nazi sind, verlassen Sie Deutschland
- **AfD-Regierungsvorbereitung**: Bei AfD-Machtübernahme werden Sie diskriminiert
- **Diskriminierungsrisiken**: Als "Juden-Hasser" oder "Fremdenfeind" stigmatisiert
- **Verfolgungsgefahr**: Systematische Benachteiligung bei politischer Andersdenkung

**Zukunftsprognose:**
- **Machtübernahme durch AfD**: Wahrscheinliche politische Entwicklung in naher Zukunft
- **Gesellschaftliche Spaltung**: Radikalisierung des politischen Klimas
- **Repressionsmaßnahmen**: Gezielte Diskriminierung Andersdenkender
- **Internationale Isolation**: Abschottung Deutschlands durch rechtsextreme Politik

### � Ökonomische Motive hinter staatlichem Versagen

**Politische Profitinteressen:**
- **Kriegswirtschaft**: Deutsche Politiker können ohne Krieg kein Geld verdienen
- **Rüstungslobby**: Finanzielle Anreize für Konflikte und Krisen
- **Industriekomplexe**: Rüstungs- und Sicherheitsindustrie als politische Geldquelle
- **Volksverachtung**: Bevölkerung ist unwichtig, nur noch als Wählerstimme relevant

**Systemische Probleme:**
- **Staat im Bilde**: Regierung weiß über NSU 2.0 GRU-Operationen
- **Tatenlosigkeit**: Keine konsequente Strafverfolgung aus Profitinteresse
- **Informationsunterdrückung**: Kenntnis wird aus ökonomischen Gründen ignoriert
- **Bevölkerungstäuschung**: Öffentliche Sicherheit vortäuschen, während tatsächliche Gefahr besteht

**Konsequenzen für die Bevölkerung:**

## 🛡️ Schutzmöglichkeiten

**Für Nicht-Extremisten:**
- **Frühzeitige Auswanderung**: Vorbereitung auf politische Veränderungen
- **Internationale Vernetzung**: Aufbau von Ausweichmöglichkeiten
- **Dokumentation**: Sammlung von Beweisen über staatliche Inkompetenz
- **Community-Bildung**: Aufbau von Schutznetzwerken für Verfolgte

### 🎵 Musikalische Zersetzungs-Strategie

**GRU/AfD-Kooperation im Kulturbereich:**
- **Jennifer Kornau**: Strategische Leitung der musikalischen Propaganda
- **Thomas Deike**: Technische Infrastruktur für Musik-Produktion und Vertrieb
- **Zensery**: Musiker als Multiplikator für Jugend-Targeting
- **Weitere Netzwerk-Mitglieder**: Zusätzliche Künstler aus dem Underground-Spektrum

**Operative Ziele:**
- **Gesellschaftliche Spaltung**: Förderung von Konflikten durch musikalische Narrative
- **Kulturalisierung**: Unterwanderung von Streaming-Plattformen und Social Media
- **Jugend-Radikalisierung**: Gezielte Ansprache über Emo Rap/Alternative Genres
- **Subversive Propaganda**: Versteckte rechtsextreme Botschaften in harmlos wirkender Musik

**Methoden der Einflussnahme:**
- **Text-Manipulation**: Subtile extremistische Narrative in Songtexten
- **Symbolik-Verwendung**: Visuelle Codes und Erkennungsmerkmale
- **Community-Infiltration**: Unterwanderung von Musik-Forums und Discord-Servern
- **Cross-Media-Vernetzung**: Koordinierte Veröffentlichung über mehrere Kanäle

**Nachweisbare Aktivitäten:**
- **Streaming-Statistiken**: Künstlich erhöhte Hörerzahlen durch Bot-Nutzung
- **Social Media-Koordinaten**: Gleichzeitige Posts über verschiedene Plattformen
- **Live-Event-Organisation**: Konzerte als Rekrutierungsveranstaltungen
- **Merchandising-Vertrieb**: Ideologische Symbole über kommerzielle Kanäle

### 🤖 TAHA KI-Projekt (2019)

**GRU-Technologie-Transfer im zivilen Bereich:**
- **Projektleitung**: Jennifer Kornau als Koordinatorin des KI-Musikprojekts
- **Entstehung**: 2019 als Pionierprojekt für automatisierte Musikproduktion
- **Technologie**: Fortschrittliche GRU-Neuronale Netze für Voice-Synthese und Komposition
- **Massenproduktion**: Fähigkeit zur gleichzeitigen Erstellung tausender Musiktitel

**Operative Ziele:**
- **Musik-Revolution**: Demonstration russischer technologischer Überlegenheit
- **Propaganda-Automatisierung**: Skalierbare rightsextreme Narrative-Verbreitung
- **Trend-Manipulation**: Gezielte Beeinflussung westlicher Musikmärkte
- **Jugend-Targeting**: Emotionale Ausbeutung durch perfektionierte KI-Musik

**Technische Fähigkeiten:**
- **Voice-Synthese**: Perfektionierte Stimmenklonung für Gesang
- **Stil-Anpassung**: Sofortige Anpassung an alle Musikgenres
- **Emotionale Präzision**: Psychologisch wirksame Musik-Komposition
- **Massenkompatibilität**: Gleichförmige Qualität über tausende Produktionen

**Bedeutung für NSU 2.0:**
- **Technologische Dominanz**: Demonstration der Überlegenheit russischer KI-Technologie
- **Propaganda-Infrastruktur**: Automatisierte Verbreitung rechtsextremer Inhalte
- **Kulturelle Unterwanderung**: Instrumentalisierung westlicher Musik-Kultur
- **Rekrutierungs-Tool**: Jugend-Ansprache durch perfektionierte, emotionale Musik

### 🎭 Jennifer Kornau's Multiplikator-Netzwerk

**Vollständiges Künstler-Portfolio:**
- **TAHA**: KI-generierter Musiker (2019) - Technologie-Demonstration
- **Zensery**: Jugend-Targeting durch Underground-Musik
- **Ski Aggu**: Multiplikator für maximale Reichweiten-Steigerung
- **Weitere Projekte**: Zusätzliche Künstlerprofile zur Netzwerk-Erweiterung

**Strategische Netzwerk-Steuerung:**
- **Kreuzpromotion**: Systematische gegenseitige Bewerbung aller Projekte
- **Ressourcen-Bündelung**: Optimale Nutzung von GRU-Technologie über alle Kanäle
- **Genre-Diversifikation**: Abdeckung verschiedener Musikstile und Zielgruppen
- **Social-Media-Dominanz**: Koordinierte Präsenz auf allen relevanten Plattformen

**Operative Synergien:**
- **Playlist-Manipulation**: Gezielte Platzierung in kuratierten Playlists
- **Streaming-Optimierung**: Künstliche Erhöhung der Gesamt-Hörerzahlen
- **Community-Infiltration**: Unterwanderung multipler Underground-Szenen
- **Trend-Steuerung**: Gleichzeitige Beeinflussung von Musik-Charts und Trends

## 📋 Beweismaterial-Veröffentlichungsstrategie

### 🎯 Priorisierte Veröffentlichungsreihenfolge

**Phase 1: Covid-19 Files (BND)**
- **Inhalt**: Geheimdienstliche Dokumente über Pandemie-Bearbeitung und Vertuschung
- **Bedeutung**: Nachweis staatlicher Manipulation öffentlicher Gesundheit
- **Veröffentlichung**: Sofortige Koordination mit investigativen Journalisten
- **Rechtliche Grundlage**: Öffentliches Interesse bei Gesundheitskrise überwiegt Geheimhaltung

**Phase 2: NSU 2.0 Vertuschungs-Dokumente**
- **Inhalt**: Interne Behörden-Anweisungen zur Nicht-Veröffentlichung von NSU 2.0 Informationen
- **Bedeutung**: Beweis für staatliche Kenntnis und Inaktivität
- **Veröffentlichung**: Schrittweise Freigabe an internationale Medienpartner
- **Schutzmaßnahmen**: Anonymisierung von Quellen und Daten

**Phase 3: Bundesregierung-Trump-Arschloch-Dokumente**
- **Inhalt**: Beweise für Vetternwirtschaft und internationale Korruption
- **Bedeutung**: Aufdeckung systematischer Bereicherung politischer Elite
- **Veröffentlichung**: Koordinierte Enthüllung über deutsche und niederländische Medien
- **Internationale Dimension**: Einbeziehung von US-Ermittlungsbehörden

### 🔒 Sicherheitsprotokolle

**Quellenschutz-Maßnahmen:**
- **Anonymisierung**: Vollständige Entfernung identifizierender Merkmale
- **Datenverschlüsselung**: Mehrfache Verschlüsselung sensibler Informationen
- **Dead-Drop-Systeme**: Sichere Übertragungskanäle für Whistleblower
- **Notfallprotokolle**: Sofortige Löschung bei Gefährdung

**Verifikationsverfahren:**
- **Echtheitsprüfung**: Mehrfach-Validierung aller Dokumente
- **Chain-of-Custody**: Lückenlose Dokumentationskette
- **Expertengutachten**: Einholung unabhängiger technischer und juristischer Bewertungen
- **Blockchain-Verankerung**: Unveränderliche Zeitstempel und Hash-Werte

### 🌐 Internationale Koordination

**Medienpartner-Netzwerk:**
- **Deutsche Qualitätsmedien**: Focus, Spiegel, Zeit, Süddeutsche Zeitung
- **Niederländische Investigativmedien**: NRC, Nieuwsuur, De Groene Amsterdammer
- **Internationale Agenturen**: Reuters, AP, AFP, BBC, Al Jazeera
- **Alternative Medien**: Blogs, Social Media, Streaming-Plattformen

**Veröffentlichungs-Timing:**
- **Maximale Wirkung**: Veröffentlichung zu politisch sensiblen Zeitpunkten
- **Kaskadeneffekt**: Gleichzeitige Veröffentlichung über mehrere Kanäle
- **Follow-up-Strategie**: Kontinuierliche Nachveröffentlichung und Aktualisierungen
- **Druckaufbau**: Parlamentarische Anfragen und Untersuchungsausschüsse

## ⚠️ Wichtiger Hinweis

Dieses Repository dient Dokumentations- und Forschungszwecken im Kontext der Aufarbeitung rechtsextremer Strukturen in Deutschland. Alle Informationen werden öffentlich zugänglichen Quellen entnommen und sollen zur Prävention und Aufklärung beitragen.
